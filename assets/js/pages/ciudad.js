$(document).ready(function() {
	...
	$('#addanew').click(function() {
		...
		$('#formregistro').attr('action', baseUrl + 'CiudadController/addRegistro');
	...
	$('#datalist').on('click', '.btnedit', function(){
		...
		$('#modalregistro').modal({backdrop: 'static'});
		$('#formregistro').attr('action', baseUrl + 'CiudadController/updateRegistro');
        $('#btnsave').text('Actualizar');
		$.ajax({
			type: 'ajax',
			method: 'get',
			url: baseUrl + "CiudadController/getRegistroEdit",
			data: {idr: idr},
			async: false,
			dataType: 'json',
...
function showAllRegistro(){
    $.ajax({
        type: 'ajax',
        method: 'get',
        url: baseUrl + "CiudadController/showAllRegistro",
        async: false,
        dataType: 'json',
        success: function(data){
            var html = '';
...
$('#formbuscarregistro').submit(function(e){
        e.preventDefault();
          ...
          var login = function(){
            $.ajax({
              type: 'POST',
              url: url + 'CiudadController/buscarRegistro',
              dataType: 'json',
              data: user,
    ...
    $('#datalist').on('click', '.btndelete', function(){
        ...
        if(confirmar == true){
            ...
            $.ajax({
                ...
                method: 'get',
                async: false,
                url: baseUrl + "CiudadController/deleteRegistro",
                data:{idreg:idu},
                success: function(respuesta){
                    location.reload();
                },
