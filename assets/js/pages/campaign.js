$(document).ready(function() {
	...
	$('#addanew').click(function() {
		...
		$('#formregistro').attr('action', baseUrl + 'CampaignController/addRegistro');
	...
	$('#datalist').on('click', '.btnedit', function(){
		...
		$('#modalregistro').modal({backdrop: 'static'});
		$('#formregistro').attr('action', baseUrl + 'CampaignController/updateRegistro');
        $('#btnsave').text('Actualizar');
		$.ajax({
			type: 'ajax',
			method: 'get',
			url: baseUrl + "CampaignController/getRegistroEdit",
			data: {idr: idr},
			async: false,
			dataType: 'json',
...
function showAllRegistro(){
    $.ajax({
        type: 'ajax',
        method: 'get',
        url: baseUrl + "CampaignController/showAllRegistro",
        async: false,
        dataType: 'json',
        success: function(data){
            var html = '';
            ...
            for(i=0, n=1; i<data.length; i++, n++){
                html +='<tr>'+
                        ...
                        '<td class="">'+data[i].descripcion+'</td>'+
...
$('#formbuscarregistro').submit(function(e){
        e.preventDefault();
          ...
          var login = function(){
            $.ajax({
              type: 'POST',
              url: url + 'CampaignController/buscarRegistro',
              dataType: 'json',
              data: user,
                  ...
                  alert('No hay lo que está buscando.');
                    ...
                    for(i=0, n=1; i<response.length; i++, n++){
                        html +='<tr>'+
                            ...
                            '<td class="">'+response[i].descripcion+'</td>'+
    ...
    $('#datalist').on('click', '.btndelete', function(){
        ...
        if(confirmar == true){
            ...
            $.ajax({
                ...
                method: 'get',
                async: false,
                url: baseUrl + "CampaignController/deleteRegistro",
                data:{idreg:idu},
                success: function(respuesta){
                    location.reload();
                },
